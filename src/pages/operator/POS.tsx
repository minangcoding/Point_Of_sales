...
export default function POS() {
  ...
  const handleCheckout = async (method: 'cash' | 'qr') => {
    ...
    try {
      ...
      if (method === 'cash') {
        ...
        alert('QR Code telah dibuat. Silakan konfirmasi pembayaran pembeli.');
        setCart([]);
        setPaymentModal(false);
      }
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 pb-24 sm:pb-4">
            {filteredProducts.map(p => (
              <div 
                key={p.id} 
                onClick={() => addToCart(p)}
                className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all cursor-pointer select-none active:scale-95 flex flex-col overflow-hidden group"
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <p className="font-bold text-blue-600 mt-2 text-sm sm:text-base">Rp {p.price.toLocaleString('id-ID')}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div id="mobile-cart" className="fixed inset-0 z-40 bg-white sm:relative sm:z-20 flex flex-col sm:flex flex-shrink-0 w-full sm:w-96 border-l border-gray-200 transition-transform duration-300 translate-y-full sm:translate-y-0 shadow-2xl sm:shadow-none">
         ...
         <div className="flex-1 overflow-y-auto p-4 space-y-4">
           {cart.length === 0 ? (
             ...
             cart.map(item => (
               <div key={item.id} className="flex flex-col gap-2 p-3 border border-gray-100 rounded-xl hover:border-gray-200 bg-gray-50/50">
                 <div className="flex justify-between items-start">
                   <h4 className="font-medium text-gray-900 truncate pr-2 leading-tight">{item.name}</h4>
                 ...
                 <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3 bg-white rounded-lg border border-gray-200 p-1">
                      <button onClick={() => updateQty(item.id, -1)} className="p-1 text-gray-500 hover:bg-gray-100 rounded">
