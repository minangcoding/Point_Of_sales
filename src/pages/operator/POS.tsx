                  ...
                  <div className="space-y-2 mb-3 max-h-32 overflow-y-auto">
                    {cart.map(item => (
                      <div key={item.id} className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-gray-900">Rp {(item.price * item.qty).toLocaleString('id-ID')}</span>
                      </div>
                    ))}
                  </div>
           ...
           <div className="bg-white w-full sm:w-[400px] sm:rounded-2xl rounded-t-2xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:fade-in-0 duration-200 flex flex-col max-h-[90vh]">
              <div className="p-6 overflow-y-auto" id="receipt-content">
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-bold text-gray-900 leading-tight">Bakso Prasmanan</h2>
                  <p className="text-gray-500 text-sm">Pakansari, Bogor</p>
                ...
                <div className="border-t border-dashed border-gray-300 py-4 mb-4">
                  <div className="space-y-3">
                    {receiptData.items.map(item => (
                      <div key={item.id} className="flex justify-between text-sm">
                        <span className="text-gray-700 flex-1 pr-4">{item.qty}x {item.name}</span>
                        <span className="font-medium text-gray-900 whitespace-nowrap">Rp {(item.price * item.qty).toLocaleString('id-ID')}</span>
                      </div>
                    ))}
                  </div>
                </div>
